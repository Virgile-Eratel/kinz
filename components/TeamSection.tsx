  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 z-20 h-[40%] bg-gradient-to-t from-charbon via-charbon/60 to-transparent pointer-events-none" />
        ...
        {/* z-40: Progress bar */}
        <div className="absolute bottom-6 left-6 right-6 z-40 h-0.5 bg-sable/20 rounded-full">
          <motion.div
            className="h-full bg-lagoon-pop rounded-full"
...
function NameSlide({
  ...
  return (
    <div
      className="min-w-[100vw] flex-shrink-0 flex flex-col justify-end p-8 md:p-16"
      role="group"
    ...
    >
      ...
      >
        {isFirst && (
          <span className="text-sm uppercase tracking-widest text-lagoon-pop mb-4 block">
            Notre equipe
          </span>
        )}
        <h2 className="text-5xl md:text-7xl font-bold text-sable leading-tight">
          {member.name}
        </h2>
        <div className="flex flex-wrap gap-2 mt-3">
          {member.roles.map((role) => (
            <span key={role} className="text-lg md:text-xl text-sable/70">
              {role}
            </span>
          ))}
        </div>
        <p className="text-sable/50 mt-4 max-w-md text-sm md:text-base md:hidden">
          {member.description}
        </p>
      </motion.div>
    </div>
  ...
  return (
    <div className="min-w-[100vw] flex-shrink-0 relative">
      <motion.div
        ...
        className="absolute bottom-0 left-1/2 -translate-x-1/2 h-[70%] md:h-[80%] w-full max-w-lg"
      >
        {/* Desktop: 3D tilt */}
